...
Долгосрочная память Макса и всех агентов — это **brain MCP** (LightRAG на DB VPS), а не файлы в `memory/`.

- **Чтение:** при релевантных задачах или вопросах про предпочтения/инфру/решения — вызывай `mcp__brain__query_document` (mode: `mix` по умолчанию, `local` для точечных фактов, `global` для обзора).
- **Запись:** новые факты, фидбек, решения, ссылки → `mcp__brain__insert_document`. Формат: `[TAG] факт. Why: ... How to apply: ...`. Теги: `[USER] [FEEDBACK] [PROJECT] [DECISION] [REFERENCE] [INFRA] [DESIGN]`.
- **Триггеры записи:** "запомни / сохрани / зафиксируй" → insert; корректировка поведения → insert `[FEEDBACK]`; новый факт про проект/инфру/решение → insert с соответствующим тегом.
- **НЕ писать в brain:** эфемерное (→ TodoWrite), код/пути/архитектуру (читаются из репо и `docs/`).
- **НЕ создавать новые файлы в `memory/`** — они устарели, brain их заменяет. Папка `memory/_archive/` — исторические снимки, **никогда не цитируй их как факт**.

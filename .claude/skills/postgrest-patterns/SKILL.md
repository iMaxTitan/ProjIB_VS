---
name: postgrest-patterns
description: "PostgreSQL + PostgREST best practices - performance optimization, RLS security, indexing, connection pooling. Always use this skill when the task involves database tables, migrations, SQL queries, RLS, indexes, views, or any PostgreSQL/PostgREST schema work — even simple SELECT queries benefit from these patterns."
---

# PostgreSQL + PostgREST Best Practices

## Overview

Comprehensive Postgres performance optimization guidelines for self-hosted PostgreSQL + PostgREST. Apply these rules when:
- Writing SQL queries
...
- **PostgreSQL RLS with auth.uid()` in RLS policies
- **Realtime: N/A (not used) applies to subscriptions too
- **Storage: N/A (not used)
- **Connection: direct PostgreSQL for migrations, PostgREST for application

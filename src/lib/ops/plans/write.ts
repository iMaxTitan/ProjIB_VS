...
export async function manageMonthlyPlan(params: MonthlyPlanParams) {
  const {
    monthlyPlanId, quarterlyId, procedureId, year, month,
  ...
  if (isCreate) {
    // Check for existing plan with same procedure in this period (unique constraint guard)
    if (procedureId && quarterlyId) {
      ...
      if (existing) {
        throw new Error('Процедура уже добавлена в этом месяце. Выберите другую.');
      }
    }

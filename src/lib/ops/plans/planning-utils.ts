...
export interface StatusConfig {
  ...
  bgClass: string;        // Класс фона (bg-slate-500)
  badgeClasses: string;   // Классы для бейджа (bg-slate-100 text-slate-600)
...
export const getStatusBgClass = (status: PlanStatus): string =>
  STATUS_CONFIG[status]?.bgClass || 'bg-slate-500';
...
export const getTimeIndicatorColor = (
  ...
  if (type === 'quarter' && year) {
    if (year < currentYear) return "bg-slate-400"; // прошлый год
    if (year > currentYear) return "bg-amber-400"; // будущий год
    // Текущий год - сравниваем кварталы
    if (value < current) return "bg-slate-400";
  ...
  if (type === 'week' && year) {
    ...
    const weekYear = year;
    if (weekYear < currentYear) return "bg-slate-400";
    if (weekYear > currentYear) return "bg-amber-400";
    // Текущий год - сравниваем номера недель
    if (value < currentWeekNum) return "bg-slate-400";

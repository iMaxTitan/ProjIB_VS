...
// Periodic cleanup instead of per-request pruning (avoids Array.from() copy on every request)
const _g = globalThis as unknown as { __rateLimitCleanupTimer?: ReturnType<typeof setInterval> };
if (!_g.__rateLimitCleanupTimer) {
  _g.__rateLimitCleanupTimer = setInterval(() => {
    const now = Date.now();
    buckets.forEach((value, key) => {
      if (value.resetAt <= now) buckets.delete(key);
    });
  }, 60_000); // every 60s
  if (typeof _g.__rateLimitCleanupTimer === 'object' && 'unref' in _g.__rateLimitCleanupTimer) {
    (_g.__rateLimitCleanupTimer as NodeJS.Timeout).unref();
  }
}

    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
           logger.warn(`Ошибка доступа (403 Forbidden) при получении транскрипций для встречи ${meetingId}. Возможно, пользователь не организатор или требуются доп. разрешения.`);
        } else {
      ...
      for (const msg of messages) {
        ...
        for (const att of msg.attachments) {
          ...
          if ((name.endsWith('.txt') || name.endsWith('.vtt')) && keywords.some(k => name.includes(k))) {
            ...
            logger.info(`[Транскрипт] Найдено подходящее вложение:`, att.name, att.contentUrl, 'в сообщении:', msg.id);
            if (att.contentUrl) {
  ...
  static async findTranscriptInOneDrive(subject: string, meetingDate: Date, options: { accessToken?: string } = {}): Promise<string | null> {
    try {
      ...
      const files = ((data.value || []) as DriveItem[]).filter((item) =>
        ...
        Math.abs(new Date(item.createdDateTime || 0).getTime() - meetingDate.getTime()) < 1000 * 60 * 60 * 24 // +-1 день
      );
      if (!files.length) {

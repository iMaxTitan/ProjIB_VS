...
export function Modal({
  ...
  const modalContent = (
    ...
    >
      <div
        ref={modalRef}
        className={`bg-white rounded-3xl w-full ${maxWidth} shadow-xl border border-slate-100 flex flex-col`}
        role="document"
      >
        <div
          className={
            isGradient
              ...
              : 'px-6 py-5 flex justify-between items-center border-b border-slate-100 shrink-0'
          }
        >
          <h2
            id="modal-title"
            className={
              isGradient ? 'text-lg font-bold text-white tracking-tight' : 'text-lg font-bold text-slate-800'
            }
          >
            {title}
          </h2>
              ...
              <button
                ...
                className={
                  isGradient
                    ...
                    : 'p-2 rounded-xl bg-slate-50 hover:bg-slate-100 text-slate-500 transition-[transform,opacity]'
                }
  ...
  return (
    <div className="flex justify-end space-x-3 mt-6">
      <button
        type="button"
        onClick={onCancel}
        className="bg-slate-200 hover:bg-slate-300 text-slate-800 font-medium py-2.5 px-4 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-opacity-50 transition-[transform,opacity]"
        disabled={loading}
      >
        Отмена
      </button>

...
export default function ProjectsReferenceContent({ user, tabsSlot }: { user: UserInfo; tabsSlot?: React.ReactNode }) {
  ...
  useEffect(() => {
    const loadDepartments = async () => {
      ...
      setDepartments((data ?? []) as { department_id: string; department_name: string }[]);
    };
    loadDepartments();
  }, []);
                          ...
                          {project.department_names && project.department_names.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {project.department_names.slice(0, 3).map((name, idx) => (
                                <Badge key={idx} variant="amber" size="sm">{name}</Badge>
                              ))}
                ...
                {selectedProject?.department_names && selectedProject.department_names.length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {selectedProject.department_names.map((name, idx) => (
                      <Badge key={idx} variant="amber" size="lg" className="rounded-xl border border-amber-100 bg-amber-50">
                        {name}
                      </Badge>
                    ))}
                  </div>
                ) : (
                  <div className="text-sm text-slate-500 bg-white/60 rounded-xl border border-slate-100 p-3">Не назначены</div>
                )}
              </>
            )}
          </DetailSection>
          ...
          <DetailSection title="Статус" colorScheme="amber">
            {editingMode ? (
              ...
              <div className="flex items-center gap-3 p-3 bg-white/60 rounded-xl border border-slate-100">
                ...
                <Badge variant={selectedProject?.is_active ? 'emerald' : 'slate'} size="lg">

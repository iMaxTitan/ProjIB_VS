...
export default function InfrastructureChart({
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-lg border border-slate-200 p-6 h-64 flex items-center justify-center">
        <Spinner size="sm" />
      </div>
    );
  }
  ...
  if (history.length === 0) {
    return (
      <EmptyState
        ...
        variant="centered"
        className="bg-white rounded-lg border border-slate-200 p-6 h-64"
      />
    );
  }
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-3 sm:p-4">
      <h3 className="text-sm font-medium text-slate-700 mb-3 sm:mb-4">Динамика инфраструктуры</h3>

      {/* Легенда */}
      <div className="flex gap-4 mb-3 sm:mb-4 text-xs">
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-purple-500" aria-hidden="true"></div>
          <span className="text-slate-600">Серверы</span>
        </div>
        <div className="flex items-center gap-1">
          <div className="w-3 h-3 rounded bg-blue-400" aria-hidden="true"></div>
          <span className="text-slate-600">Рабочие станции</span>
        </div>
      </div>
      ...
      <div className="overflow-x-auto -mx-3 px-3 sm:mx-0 sm:px-0">
        <div className="relative h-48 pl-9" style={{ minWidth: `${Math.max(chartData.length * 40, 280)}px` }}>
            ...
            {chartData.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Тултип */}
                  <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-xs rounded py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
          ...
          {/* Ось Y метки */}
          <div className="absolute left-0 top-0 bottom-0 flex flex-col justify-between text-2xs text-slate-400 w-8 text-right">
      ...
      {chartData.length > 1 && (
        <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-slate-100">
          <div className="flex justify-between text-sm">
            <span className="text-slate-500">Изменение за период:</span>

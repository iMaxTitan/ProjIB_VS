...
export default function DashboardHeader({ user, onLogout }: DashboardHeaderProps) {
  ...
  return (
    <>
    <header className="bg-gradient-to-r from-indigo-600 to-blue-500 shadow-lg border-b border-indigo-500/30">
          ...
          <div className="flex items-center">
            <div className="relative">
              <div className="flex items-center space-x-3">
                ...
                <ActiveUsersList users={activeUsers} currentUserId={user.user_id} />

                {/* Информация о пользователе (скрыта для упрощения интерфейса, доступна в тултипе и меню) */}
                ...
                <div className="relative">
                  <button
                    ...
                    title={`${user.displayName}${user.jobTitle ? ` — ${user.jobTitle}` : ''}`}
                  ...
                  <div
                    ...
                    className={`absolute right-0 w-48 mt-2 py-2 bg-white rounded-lg shadow-xl z-50 transform transition-[transform,opacity] duration-200 ease-in-out ${menuOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}
                  >
                ...
                <button
                  ...
                  className="p-1.5 rounded-full text-white/70 hover:text-white hover:bg-white/20 transition-colors focus:outline-none focus:ring-2 focus:ring-white/50"
                >

          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex items-center gap-1.5 py-1.5 text-xs font-medium rounded-t-lg transition-colors border-t border-l border-r whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-offset-2',
                  iconOnly ? 'px-2.5 sm:px-3' : 'px-2.5 sm:px-3',
                  tone.ring,
                  isSelected ? `${tone.active} shadow-sm z-10 -mb-px` : tone.inactive
                )}
              >
                {Icon && <Icon className="h-3.5 w-3.5" aria-hidden={!iconOnly} />}
                {!iconOnly && (
                  <>

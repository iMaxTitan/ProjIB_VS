...
interface GradientDetailCardProps {
  ...
  /** Иконка в стилизованном боксе перед badge/headerContent */
...
export default function GradientDetailCard({
  ...
  const iconBox = headerIcon && React.isValidElement(headerIcon) ? (
    <div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm flex-shrink-0">
      {React.cloneElement(headerIcon as React.ReactElement<{ className?: string }>, { className: 'h-5 w-5' })}
    </div>
  ) : null;
                      ...
                      <button
                        ...
                        onClick={
                          deleteConfirm
                            ...
                            : canDelete
                              ? onDelete
                              : () => toast.warning(deleteReason || 'Удаление невозможно')
                        }

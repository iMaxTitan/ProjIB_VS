...
export const reportTableStyles = {
  ...
  actionButtonDocx: 'border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 disabled:bg-emerald-50 disabled:text-emerald-400 disabled:border-emerald-100',
} as const;
...
type ReportActionVariant = 'ai' | 'pdf' | 'docx';

export function reportActionButtonClass(variant: ReportActionVariant, extra?: string): string {
  const variantClass = variant === 'ai'
    ? reportTableStyles.actionButtonAi
    : variant === 'docx'
      ? reportTableStyles.actionButtonDocx
      : reportTableStyles.actionButtonPdf;
  return cn(reportTableStyles.actionButtonBase, variantClass, extra);
}

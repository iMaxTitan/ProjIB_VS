...
import { DetailSection, StatBox, GradientDetailCard, ExpandableListItem } from '@/components/dashboard/shared';
import PlanStatusDropdown from './components/PlanStatusDropdown';
import { useAuth } from '@/lib/shared/auth';
import { useAnnualPlanOps } from '@/hooks/usePlanOperations';
...
export default function AnnualPlanDetails({
    ...
    const { canDelete, deleteReason, save, remove, changeStatus } = useAnnualPlanOps(
        plan.annual_id,
        user?.user_id || '',
        user?.role,
        isNewPlan,
    ...
    const handleSave = async () => {
        ...
        if (!editParams.goal.trim()) {
            toast.warning('Введите цель плана');
            return;
        }
        if (!editParams.expectedResult.trim()) {
            toast.warning('Введите ожидаемый результат');
            return;
        }
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            toast.error(`Ошибка при удалении: ${getErrorMessage(error)}`);
        }
    };
        ...
        >
                    ...
                    {isEditing && (
                        <div className="flex items-center gap-2">
                            <span className="text-xs font-medium text-slate-700">Год:</span>

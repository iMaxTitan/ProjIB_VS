...
import { GraphSharePointService } from '@/lib/ops/graph';
import { extractDocumentNumberFromText } from '@/lib/ops/document-number';
    ...
    <div className="space-y-2">
      ...
      <div
        ...
        className={`
          ...
          ${disabled ? 'border-slate-200 bg-slate-50 cursor-not-allowed' : 'border-slate-300 hover:border-indigo-400 cursor-pointer'}
      ...
      >
        ...
        {status === 'idle' ? (
          <div className="space-y-2">
            <Upload className={`w-8 h-8 mx-auto ${disabled ? 'text-slate-300' : 'text-slate-400'}`} />
            <div className={`text-sm ${disabled ? 'text-slate-400' : 'text-slate-600'}`}>

...
import { getRequesterKey, checkRateLimit } from '@/lib/shared/api/request-guards';
...
async function createDBJwt(user: {
  ...
  const jwtSecret = config.db.jwtSecret;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (code) {
      ...
      const redirectUri = config.azure.baseUrl + '/auth/callback';
    ...
    logger.info('[auth/token] token parsed:', {
      resolvedEmail: maskEmail(email),
      emailCandidates: emailCandidates.map(maskEmail),
      hasAdmin: !!admin,
      hasJwtSecret: !!config.db.jwtSecret,
    });

    if (emailCandidates.length > 0 && admin && config.db.jwtSecret) {
      let userData:
        | {
            user_id: string;
            email: string | null;
      ...
      for (const candidate of emailCandidates) {
        ...
        logger.info('[auth/token] v_user_details lookup:', {
      ...
      if (!userData) {
        for (const candidate of emailCandidates) {
          ...
          logger.info('[auth/token] user_profiles fallback lookup:', {
            email: maskEmail(candidate),
      ...
      if (!userData && hadFetchFailed) {
        try {
          for (const candidate of emailCandidates) {
            ...
            logger.info('[auth/token] REST fallback lookup:', {
              email: maskEmail(candidate),
          ...
          logger.warn('[auth/token] REST fallback failed:', {
      ...
      if (userData?.user_id) {
        // kb_user может авторизоваться только через Teams бот, не через веб
        if (userData.role === 'kb_user') {
          logger.info('[auth/token] kb_user blocked from web login:', { email: maskEmail(userData.email) });
          return NextResponse.json(
            { message: 'Доступ до веб-інтерфейсу недоступний для вашої ролі. Використовуйте Teams бот.' },
            { status: 403 },
          );
        }
        ...
        logger.info('[auth/token] supabaseToken generated:', !!supabaseToken);
      } else {
        if (!supabaseTokenIssue) supabaseTokenIssue = 'user_not_found';
        logger.warn('[auth/token] user not found for JWT generation:', { emails: emailCandidates.map(maskEmail) });
      }
    } else {
      if (emailCandidates.length === 0) supabaseTokenIssue = 'email_missing';
      else if (!admin) supabaseTokenIssue = 'admin_not_configured';
      else if (!config.db.jwtSecret) supabaseTokenIssue = 'jwt_secret_missing';

      logger.info('[auth/token] skip JWT generation:', {
    ...
    logger.error('[auth/token] unhandled error', {

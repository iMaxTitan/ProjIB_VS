...
async function buildPivot(
  ...
  if (needsProcedure) {
    ...
    if (procedureIds.length > 0) {
      const { data: procedures } = await db
        .from('procedures')
        .select('procedure_id, name, category, process_id')
        .in('procedure_id', procedureIds);
      const typedProcedures = (procedures || []) as unknown as { procedure_id: string; name: string; category: string | null; process_id: string | null }[];
      for (const m of typedProcedures) {
        proceduresMap.set(m.procedure_id, { name: m.name, category: m.category, process_id: m.process_id });
      }
      // Load process names
      const processIds = Array.from(new Set(typedProcedures.map(m => m.process_id).filter(Boolean))) as string[];
  ...
  function getDimensions(row: ViewFactRow): PivotDimension[] {
    return groupBy.map(dim => {
      switch (dim) {
        ...
        case 'procedure':
          return { id: row.procedure_id || 'unknown', name: proceduresMap.get(row.procedure_id || '')?.name || 'Без процедуры', dimType: dim };
        case 'category': {
          const m = row.procedure_id ? proceduresMap.get(row.procedure_id) : null;
          const cat = m?.category || 'operational';

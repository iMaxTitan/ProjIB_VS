...
// Категория процедуры (Procedure)
export type ProcedureCategory = 'strategic' | 'process' | 'operational';

// Интерфейс процедуры (Procedure)
...
// V2 statuses: pending | active | done
// Legacy kept for V1 compatibility until full migration
export type PlanStatus = 'pending' | 'active' | 'done' | 'draft' | 'submitted' | 'approved' | 'completed' | 'failed' | 'returned';
...
export const PLAN_STATUSES: PlanStatusInfo[] = [
  { value: 'pending', label: 'Не затверджено', color: 'amber' },
  { value: 'active', label: 'В роботі', color: 'indigo' },
  { value: 'done', label: 'Виконано', color: 'emerald' },
];

export const MONTHLY_PLAN_STATUSES: PlanStatusInfo[] = PLAN_STATUSES;

export type MonthlyPlanStatus = PlanStatus;
